...
A sample dataset of Input-Output tables is included.
In order to save space and speed up computations, the included dataset uses regional aggregates.
...
in the sample data we have **G = 7** (generally countries, here regions) and **N = 43** (number of industries).
```{r data-dimensions}
dim(intermediate_demand) # (2 + GN + totals) x (2 + GN)
dim(final_demand)        # (2 + GN + totals) x (2 + G*5)
```

Note thata **G** and **N** are automatically inferred from the dimensions of the data.

When importing data, be sure to specify **stringsAsFactors = FALSE** and to **header=FALSE**,
as the Input-Output tables use two header lines (country and industry),
