...
the field.
The R package decompr solves this problem by implementing the algorithms for the analysis of Global Value Chains as R procedures, thereby simplifying the decomposition process.
...
It derives the value added origins of an industry's exports by source country and source industry, using easily vailable gross trade data.
...
Using the results of the two decompositions, decompr provides a set of Global Value Chain indicators, such as the now standard Vertical Specialisation ratio.
...
\section*{Colophon}
This paper was written in a combination of R \parencite{core2014r} and \LaTeX ~\parencite{lamport1986document},
specifically \LuaTeX ~\parencite{hagen2005luatex}, with biblatex and biber \parencite{lehman2006biblatex} for citations,
using Sweave syntax \parencite{leisch2003sweave} and compiled using knitr \parencite{yihui2013knitr} and Pandoc \parencite{macfarlane2012pandoc}.
