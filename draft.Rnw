...
the complex matrix transformations at the basis of most Value Chain indicators still constitute a significant entry barrier to
the field. The R package \textbf{decompr} solves this problem by implementing algorithms for the analysis of Global Value Chains,
which calculate standard indicators in an automated fashion. Two methods for gross export flow decomposition using 
Inter-Country Input-Output tables are provided. The first method is the \textbf{Wang-Wei-Zhu} algorithm which splits 
...
The \textbf{Leontief} decomposition of gross trade flows solves this problem
...
\section{Data}
\label{sec:data}
Two data sets are included in the package, one real world data set, and one minimal fictional data for training purposes.
The former is the WIOD regional Inter-Country Input Output table for the year 2011 \parencite{timmer2012world}. 
The latter is a fictional 3-country 3-sector data set, which we will use throughout this article to exemplify
the mechanics of the package. The data can be accessed as follows:
...
provided in ICIOS. More specifically, the Leontief decomposition traces the value added back to where it originates
but ICIOS also contain data on how the value added is subsequently used. This information is extracted by the
Wang-Wei-Zhu decomposition, which thereby allows a much more detailed look at the structures of international
production networks and the respective positions of countries and industries within them.
